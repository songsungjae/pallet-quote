                ...
                return(
                  <div>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
                      ...
                      <span style={{marginLeft:"auto",...S.tag("amber")}}>중량 전용</span>
                    </div>
                    <div style={{background:"#fffbeb",border:"1px solid #fcd34d",borderRadius:8,padding:"12px 16px",marginBottom:12,fontSize:12,color:"#92400e"}}>
                      ⚖️ 사이즈 없이 중량으로 계산 · 파렛트당 총 중량 {fmt(weightOnlyMaxKg)}kg 한도 · 파렛트: {p?.name}
                    </div>
                    {estQ?(
                      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
                        <div style={S.resultCard(C.amber)}><div style={{fontSize:11,color:C.textSub,marginBottom:4}}>적입수량 (중량 기준)</div><div style={{fontSize:20,fontWeight:700,color:C.amber}}>{fmt(estQ)}</div><div style={{fontSize:11,color:C.textMuted}}>{weightOnlyMaxKg}kg ÷ {m}kg</div></div>
                        <div style={S.resultCard(C.amber)}><div style={{fontSize:11,color:C.textSub,marginBottom:4}}>개당 단가</div><div style={{fontSize:20,fontWeight:700,color:C.amber}}>₩{fmt(eUC)}</div><div style={{fontSize:11,color:C.textMuted}}>{p?.name}</div></div>
                        <div style={S.resultCard(C.amber)}><div style={{fontSize:11,color:C.textSub,marginBottom:4}}>연간 포장비</div><div style={{fontSize:20,fontWeight:700,color:C.amber}}>₩{fmt(p?p.price*ePC:0)}</div><div style={{fontSize:11,color:C.textMuted}}>파렛트 {fmt(ePC)}개</div></div>
                      </div>
                    ):<div style={S.msg}>중량 정보를 입력하세요</div>}
                  </div>
                );
              }
              const{p,best,qpp,estQ,corrRatio,exactMatch,exactCount,tagMatchCount,noData,aq}=r;
              const cUC=qpp>0?parseFloat((p.price/qpp).toFixed(2)):0,eUC=estQ?parseFloat((p.price/estQ).toFixed(2)):0;
            ...
            <div style={S.section}>
              ...
              {qResult.qpp<=0?<div style={S.msg}>어떤 방향으로도 들어가지 않습니다.</div>:!qResult.aq?<div style={S.msg}>연간 생산수량을 입력하세요</div>:(()=>{
                ...
                return(
                  <>
                    {/* 요약 KPI 바 */}
                    <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:10,marginBottom:14,background:"#f8fafc",borderRadius:10,padding:"12px 14px"}}>
                      <div style={{textAlign:"center"}}>
                        <div style={{fontSize:10,color:C.textSub,fontWeight:600,marginBottom:3}}>이론 적입수량</div>
                        <div style={{fontSize:20,fontWeight:700,color:C.blue}}>{fmt(qpp)}</div>
                        <div style={{fontSize:10,color:C.textMuted}}>개/파렛트</div>
                      </div>
                      <div style={{textAlign:"center"}}>
                        <div style={{fontSize:10,color:C.textSub,fontWeight:600,marginBottom:3}}>이론 개당 단가</div>
                        <div style={{fontSize:20,fontWeight:700,color:C.blue}}>₩{fmtP(cUC)}</div>
                        <div style={{fontSize:10,color:C.textMuted}}>{p.name}</div>
                      </div>
                      <div style={{textAlign:"center",borderLeft:`1px solid ${C.border}`,borderRight:`1px solid ${C.border}`}}>
                        <div style={{fontSize:10,color:C.textSub,fontWeight:600,marginBottom:3}}>추정 적입수량</div>
                        <div style={{fontSize:20,fontWeight:700,color:estQ?C.green:C.textMuted}}>{estQ?fmt(estQ):"—"}</div>
                        <div style={{fontSize:10,color:C.textMuted}}>개/파렛트</div>
                      </div>
                      <div style={{textAlign:"center"}}>
                        <div style={{fontSize:10,color:C.textSub,fontWeight:600,marginBottom:3}}>추정 개당 단가</div>
                        <div style={{fontSize:20,fontWeight:700,color:estQ?C.green:C.textMuted}}>{estQ?`₩${fmtP(parseFloat((p.price/estQ).toFixed(2)))}`:"—"}</div>
                        <div style={{fontSize:10,color:C.textMuted}}>{p.name}</div>
                      </div>
                      <div style={{textAlign:"center"}}>
                        <div style={{fontSize:10,color:C.textSub,fontWeight:600,marginBottom:3}}>연간 총 포장비</div>
                        <div style={{fontSize:18,fontWeight:700,color:C.amber}}>{estQ||qpp?`₩${fmt(Math.round(p.price*Math.ceil(aq/(estQ||qpp))/10000))}만`:"—"}</div>
                        <div style={{fontSize:10,color:C.textMuted}}>추정 단가 기준</div>
                      </div>
                    </div>
                    ...
                    <div style={S.resultGrid}>
                      ...
                      <div style={S.resultCard(C.blue)}><div style={{fontSize:11,color:C.textSub,marginBottom:5}}>이론 개당 단가</div><div style={{fontSize:22,fontWeight:700,color:C.blue}}>₩{fmtP(cUC)}</div><div style={{fontSize:11,color:C.textMuted}}>{p.name}</div></div>
                      ...
                      return(
                        <>
                          ...
                          <div style={S.resultGrid}>
                            ...
                            <div style={S.resultCard(color)}><div style={{fontSize:11,color:C.textSub,marginBottom:5}}>{isExact?"실적 개당 단가":"추정 개당 단가"}</div><div style={{fontSize:22,fontWeight:700,color}}>₩{fmtP(eUC)}</div><div style={{fontSize:11,color:C.textMuted}}>{p.name}</div></div>
