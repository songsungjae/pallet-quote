...
const fmtP=n=>parseFloat(n.toFixed(2)).toLocaleString("ko-KR",{minimumFractionDigits:2,maximumFractionDigits:2}); // 개당 단가 소수점 2자리
...
// 중량 전용 대분류 (사이즈 없이 중량으로만 계산)
const WEIGHT_ONLY_CATEGORIES=["H/W","PIPE NUT"];
const WEIGHT_ONLY_PALLET_ID=7; // 방청 렌탈 미니 (한계 500kg, 자중 43kg)
const WEIGHT_ONLY_MAX_KG=457;  // 실적재 가능 중량 (500kg - 43kg)

function calcOneProduct(prod,db){
  const pw=+prod.w,pd=+prod.d,ph=+prod.h,mass=+prod.mass,aq=+prod.annualQty;

  // ── 중량 전용 품목 (H/W, PIPE NUT) ──────────────────────
  const isWeightOnly=WEIGHT_ONLY_CATEGORIES.includes(prod.category)||WEIGHT_ONLY_CATEGORIES.includes(prod.tag);
  if(isWeightOnly){
    if(!mass||mass<=0)return{prod,error:"중량 정보가 없습니다",weightOnly:true};
    const p=PALLETS.find(x=>x.id===WEIGHT_ONLY_PALLET_ID);
    if(!p)return{prod,error:"파렛트 없음",weightOnly:true};
    const estQ=Math.floor(WEIGHT_ONLY_MAX_KG/mass);
    return{
      prod,p,qpp:null,best:null,
      estQ,corrRatio:null,exactMatch:false,tagMatchCount:0,
      pVol:null,pv:null,net:WEIGHT_ONLY_MAX_KG,mass,aq,
      weightOnly:true,weightOnlyMaxKg:WEIGHT_ONLY_MAX_KG,
    };
  }
  ...
  const ratios=fillSource.map(m=>{
    const mPallet=PALLETS.find(x=>x.id===m.palletId);
    if(!mPallet)return null;
    const mO=getOrients(mPallet,[+m.w,+m.d,+m.h],+m.mass||0);
    const mB=getBest(mO);
...
export default function App(){
  ...
  const[tagImages,setTagImages]=useState({}); // {tag: base64} 소분류 대표 사진
  ...
  function parseExcel(file){
    ...
    r.onload=e=>{
      try{
        ...
        const items=rows.map((row,i)=>({_idx:i,partNo:String(getVal(row,colMap.partNo)||""),name:String(getVal(row,colMap.name)||""),category:String(getVal(row,colMap.category)||""),tag:String(getVal(row,colMap.tag)||""),w:+getVal(row,colMap.w)||0,d:+getVal(row,colMap.d)||0,h:+getVal(row,colMap.h)||0,mass:+getVal(row,colMap.mass)||0,annualQty:+getVal(row,colMap.annualQty)||0}))
        .filter(x=>{
          const isWeightOnly=WEIGHT_ONLY_CATEGORIES.includes(x.category)||WEIGHT_ONLY_CATEGORIES.includes(x.tag);
          // H/W, PIPE NUT는 사이즈 없어도 허용 (중량만 있으면 됨)
          if(isWeightOnly) return x.mass>0;
          return x.w>0&&x.d>0&&x.h>0;
        });
  ...
  function downloadExcel(){
    ...
    const rows=batchItems.map((item,i)=>{
      ...
      // 중량 전용 품목
      if(r.weightOnly){
        const{p,estQ,mass:m,aq}=r;
        const eUC=estQ&&p?parseFloat((p.price/estQ).toFixed(2)):0;
        const ePC=estQ&&aq>0?Math.ceil(aq/estQ):0;
        return{품번:item.partNo,품명:item.name,대분류:item.category,소분류:item.tag,가로:"",세로:"",높이:"",중량:m,연간수량:aq,파렛트:p?p.name:"",이론적입수:"",이론개당단가:"",추정적입수:estQ||"",추정개당단가:estQ?eUC:"",연간포장비:estQ&&p?p.price*ePC:"",비고:`중량 전용 (${r.weightOnlyMaxKg}kg 한도)`};
      }
      const{p,best,qpp,estQ,corrRatio,exactMatch,exactCount,tagMatchCount,noData,aq}=r;
      const cUC=qpp>0?parseFloat((p.price/qpp).toFixed(2)):0;
      const eUC=estQ?parseFloat((p.price/estQ).toFixed(2)):0;
      const ePC=estQ&&aq>0?Math.ceil(aq/estQ):0;
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  ...
  function replaceDbFromExcel(file){
    ...
    reader.onload=e=>{
      try{
        ...
        const wb=XLSX.read(new Uint8Array(e.target.result),{type:"array"});
        const ws=wb.Sheets[wb.SheetNames[0]];
        // 1행이 타이틀인 경우를 대비해 두 가지 방식으로 시도
        const rowsH0=XLSX.utils.sheet_to_json(ws,{defval:""});
        const rowsH1=XLSX.utils.sheet_to_json(ws,{defval:"",range:1}); // 2행부터 헤더
        ...
        // 두 방식 중 더 많은 유효 데이터가 나오는 걸 사용
        const newDbH0=parseRows(rowsH0);
        const newDbH1=parseRows(rowsH1);
        const newDb=newDbH1.length>=newDbH0.length?newDbH1:newDbH0;
        if(newDb.length===0){alert("유효한 데이터가 없습니다.\n컬럼명 확인: 품번, 품명, 대분류, 소분류, 가로, 세로, 높이, 중량, 파렛트, 실적입수량");return;}
  ...
  useEffect(()=>{
    ...
    // 보정비율: 각 유사 제품의 실제 파렛트 기준으로 이론값 계산
    const ratios=fillSource.map(m=>{
      const mPallet=PALLETS.find(x=>x.id===m.palletId);
      if(!mPallet)return null;
      const mO=getOrients(mPallet,[+m.w,+m.d,+m.h],+m.mass||0);
...
// ── DB 탭 ─────────────────────────────────────────────────
function DBTab({db,pageDb,filteredDb,dbPage,totalPages,setDbPage,dbSearch,setDbSearch,dbCatFilter,setDbCatFilter,allCategories,dbForm,setDbForm,allTags,saveDb,deleteDb,editingDbItem,startEditDb,saveEditDb,setEditingDbItem,handleDbImg,PALLETS,replaceDbFromExcel,tagImages,setTagImages}){
  const fileRef=useRef();
  const dbReplaceRef=useRef();
  const[dbSubTab,setDbSubTab]=useState("list"); // "list" | "tagimg"
  const[tagImgCat,setTagImgCat]=useState("");
  ...
  // 소분류별 대표 사진 업로드
  function handleTagImage(tag,file){
    if(!file)return;
    const r=new FileReader();
    r.onload=e=>setTagImages(prev=>({...prev,[tag]:e.target.result}));
    r.readAsDataURL(file);
  }

  // 대분류 필터된 소분류 목록
  const allTagsAll=[...new Set(db.map(x=>x.tag).filter(Boolean))].sort();
  const filteredTagList=tagImgCat
    ?[...new Set(db.filter(x=>x.category===tagImgCat).map(x=>x.tag).filter(Boolean))].sort()
    :allTagsAll;
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",gap:4,marginBottom:16,borderBottom:`1px solid ${C.border}`,paddingBottom:0}}>
        {[["list","📋 제품 목록"],["tagimg","🖼 소분류 대표 사진"]].map(([key,label])=>(
          <button key={key} onClick={()=>setDbSubTab(key)} style={{padding:"8px 18px",border:"none",borderBottom:`2px solid ${dbSubTab===key?C.blue:"transparent"}`,background:"transparent",fontSize:13,fontWeight:dbSubTab===key?700:400,color:dbSubTab===key?C.blue:C.textSub,cursor:"pointer"}}>
            {label}
          </button>
        ))}
      </div>
      ...
      {dbSubTab==="tagimg"?(
        ...
        <div>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
            <select value={tagImgCat} onChange={e=>setTagImgCat(e.target.value)} style={{...S.select,width:160}}>
              <option value="">전체 대분류</option>
              {allCategories.map(c=><option key={c} value={c}>{c}</option>)}
            </select>
            <span style={{fontSize:12,color:C.textSub}}>{filteredTagList.length}개 소분류 · 클릭하여 사진 업로드</span>
            {Object.keys(tagImages).length>0&&(
              <button onClick={()=>{if(window.confirm("등록된 모든 소분류 사진을 삭제하시겠습니까?"))setTagImages({});}} style={{...S.btn("danger"),fontSize:12,marginLeft:"auto"}}>전체 삭제</button>
            )}
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:12}}>
            {filteredTagList.map(tag=>{
              const img=tagImages[tag];
              const catOfTag=db.find(x=>x.tag===tag)?.category||"";
              return(
                <div key={tag} style={{background:C.card,border:`1.5px solid ${img?C.green:C.border}`,borderRadius:10,overflow:"hidden",position:"relative"}}>
                  <label style={{cursor:"pointer",display:"block"}}>
                    {img?(
                      <img src={img} style={{width:"100%",height:120,objectFit:"cover",display:"block"}}/>
                    ):(
                      <div style={{width:"100%",height:120,background:"#f8fafc",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:6}}>
                        <span style={{fontSize:28,color:C.textMuted}}>📷</span>
                        <span style={{fontSize:10,color:C.textMuted}}>사진 업로드</span>
                      </div>
                    )}
                    <input type="file" accept="image/*" style={{display:"none"}} onChange={e=>handleTagImage(tag,e.target.files[0])}/>
                  </label>
                  <div style={{padding:"8px 10px"}}>
                    <div style={{fontSize:11,background:"#e0e7ff",color:"#3730a3",borderRadius:3,padding:"1px 5px",display:"inline-block",marginBottom:4}}>{catOfTag}</div>
                    <div style={{fontSize:13,fontWeight:600,color:C.text}}>{tag}</div>
                    {img&&(
                      <button onClick={()=>setTagImages(prev=>{const n={...prev};delete n[tag];return n;})}
                        style={{marginTop:5,background:"none",border:"none",cursor:"pointer",fontSize:11,color:C.red,padding:0}}>✕ 사진 삭제</button>
                    )}
                  </div>
                </div>
              );
...
// ── 견적 탭 ───────────────────────────────────────────────
function QuoteTab({quote,setQ,analyzing,handleQuoteImg,matched,qResult,selPallet,kpiQpp,kpiEstQ,kpiUC,kpiEstUC,kpiTotal,kpiAq,allCategories,quoteTags,batchItems,batchResults,selectedIdx,setSelectedIdx,batchLoading,batchFileRef,parseExcel,downloadExcel,downloadSample,PALLETS,getPalletWithPrice,palletPrices,editingPriceId,startEditPrice,commitPrice,setEditingPriceVal,editingPriceVal,db,tagImages}){
  return(
    <div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"200px 1fr",gap:14,marginBottom:14}}>
          <div style={{...S.section,padding:"10px",maxHeight:400,overflowY:"auto",marginBottom:0}}>
            ...
            {batchItems.map((item,i)=>{
              ...
              const dot=r?.error?C.red:r?.weightOnly?C.amber:r?.noData?C.amber:r?.exactMatch?C.purple:r?.estQ?C.green:C.blue;
              return(
                <div key={i} onClick={()=>{
                  setSelectedIdx(i);
                  // 선택된 품번 정보를 quote state에 반영 → KPI카드·매칭패널 자동 업데이트
                  setQ("category",item.category||"");
                  setQ("tag",item.tag||"");
                  setQ("w",item.w>0?String(item.w):"");
                  setQ("d",item.d>0?String(item.d):"");
                  setQ("h",item.h>0?String(item.h):"");
                  setQ("mass",item.mass>0?String(item.mass):"");
                  setQ("annualQty",item.annualQty>0?String(item.annualQty):"");
                  if(r?.p?.id)setQ("palletId",r.p.id);
                }} style={{display:"flex",alignItems:"center",gap:7,padding:"7px 8px",borderRadius:6,cursor:"pointer",background:isSel?"#eff6ff":"transparent",border:isSel?`1px solid ${C.blue}`:"1px solid transparent",marginBottom:3}}>
          ...
          <div style={{...S.section,marginBottom:0}}>
            {selectedIdx===null?<div style={S.msg}>좌측에서 품번을 선택하세요</div>:(()=>{
              ...
              // 파렛트 변경 실시간 반영 위해 qResult 우선 사용, 없으면 batchResults 사용
              const r=batchResults[selectedIdx];
              if(!r&&!qResult)return<div style={{...S.msg,color:C.red}}>계산 오류</div>;

              // 중량 전용 품목은 batchResults 고정값 사용
              if(r?.weightOnly){
                const{p,estQ,mass:m,aq,weightOnlyMaxKg}=r;
                const eUC=estQ&&p?parseFloat((p.price/estQ).toFixed(2)):0;
                const ePC=estQ&&aq>0?Math.ceil(aq/estQ):0;
                return(
                  <div>
                    <div style={{marginBottom:10}}>
                      ...
                      <div style={{fontSize:12,color:C.textSub,marginTop:3}}>
                        ...
                        {item.tag} · 중량 {m}kg · {p?.name}
                        <span style={{...S.tag("amber"),marginLeft:8}}>중량 전용</span>
                      </div>
                    </div>
                    <div style={{background:"#fffbeb",border:"1px solid #fcd34d",borderRadius:7,padding:"10px 12px",marginBottom:10,fontSize:12,color:"#92400e"}}>
                      ⚖️ {weightOnlyMaxKg}kg 한도 · {fmt(weightOnlyMaxKg)}kg ÷ {m}kg = <b>{fmt(estQ)}개/파렛트</b>
                    </div>
                    {estQ&&(
                      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
                        <div style={S.resultCard(C.amber)}><div style={{fontSize:11,color:C.textSub,marginBottom:4}}>적입수량</div><div style={{fontSize:22,fontWeight:700,color:C.amber}}>{fmt(estQ)}</div><div style={{fontSize:11,color:C.textMuted}}>개/파렛트</div></div>
                        <div style={S.resultCard(C.amber)}><div style={{fontSize:11,color:C.textSub,marginBottom:4}}>개당 단가</div><div style={{fontSize:22,fontWeight:700,color:C.amber}}>₩{fmtP(eUC)}</div><div style={{fontSize:11,color:C.textMuted}}>{p?.name}</div></div>
                        <div style={S.resultCard(C.amber)}><div style={{fontSize:11,color:C.textSub,marginBottom:4}}>연간 포장비</div><div style={{fontSize:22,fontWeight:700,color:C.amber}}>₩{fmt(p?p.price*ePC:0)}</div><div style={{fontSize:11,color:C.textMuted}}>파렛트 {fmt(ePC)}개</div></div>
                      </div>
                    )}
                  </div>
                );
              ...
              // 일반 품목: qResult 실시간값 사용 (파렛트 변경 반영)
              const liveR=qResult;
              if(!liveR)return<div style={S.msg}>좌측에서 품번을 선택하세요</div>;
              const{p,best,qpp,estQ,corrRatio,exactMatch,exactCount,tagMatchCount,noData,aq}=liveR;
              if(!p||!best)return<div style={{...S.msg,color:C.red}}>계산 오류</div>;
              const cUC=qpp>0?parseFloat((p.price/qpp).toFixed(2)):0;
              const eUC=estQ?parseFloat((p.price/estQ).toFixed(2)):0;
              const cPC=qpp>0&&aq>0?Math.ceil(aq/qpp):0;
              const ePC=estQ&&aq>0?Math.ceil(aq/estQ):0;
              const color=exactMatch?C.amber:C.green;
              return(
                <div>
                  {/* 품번 헤더 */}
                  <div style={{marginBottom:12}}>
                    <div style={{fontSize:14,fontWeight:700,color:C.text}}>{item.partNo} <span style={{fontSize:12,fontWeight:400,color:C.textSub}}>{item.name}</span></div>
                    <div style={{fontSize:12,color:C.textSub,marginTop:3,display:"flex",alignItems:"center",flexWrap:"wrap",gap:6}}>
                      <span style={{background:"#e0e7ff",color:"#3730a3",borderRadius:3,padding:"1px 6px",fontSize:11,fontWeight:600}}>{item.category}</span>
                      <span>{item.tag} · {item.w}×{item.d}×{item.h}mm · {item.mass}kg · {p.name}</span>
                      <span style={{...S.tag(exactMatch?"amber":noData?"amber":"blue"),marginLeft:"auto"}}>{exactMatch?"실적값 적용":noData?"비교군 없음":`보정비율 ${corrRatio?.toFixed(2)||"—"}`}</span>
                    </div>
                  </div>
                  {/* 이론값 */}
                  <div style={{fontSize:11,color:C.textSub,fontWeight:600,marginBottom:6}}>이론값 <span style={S.tag("blue")}>{best.l} 방향</span></div>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:8}}>
                    <div style={S.resultCard(C.blue)}><div style={{fontSize:11,color:C.textSub,marginBottom:4}}>이론 적입수량</div><div style={{fontSize:22,fontWeight:700,color:C.blue}}>{fmt(qpp)}</div><div style={{fontSize:11,color:C.textMuted}}>{best.cW}×{best.cD}×{best.cH}</div></div>
                    <div style={S.resultCard(C.blue)}><div style={{fontSize:11,color:C.textSub,marginBottom:4}}>이론 개당 단가</div><div style={{fontSize:22,fontWeight:700,color:C.blue}}>₩{fmtP(cUC)}</div><div style={{fontSize:11,color:C.textMuted}}>파렛트 {fmt(cPC)}개</div></div>
                  ...
                  {/* 이론값 기준 적재 중량 바 */}
                  {liveR.mass>0&&qpp>0&&(()=>{
                    const tw=qpp*liveR.mass,pct=Math.min(100,tw/liveR.net*100);
                    const bc=pct>=100?C.red:pct>=80?C.amber:C.green;
                    return(
                      <div style={{...S.weightBar,marginBottom:10}}>
                        <div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:C.textSub,marginBottom:2}}>
                          <span>제품 적재 중량 (이론값 기준)</span>
                          <span style={{color:bc,fontWeight:600}}>{fmtD(tw)}kg / {fmt(liveR.net)}kg</span>
                        </div>
                        ...
                        <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:C.textMuted}}>
                          <span>한계 {fmt(p.maxKg)}kg − 자중 {p.tare}kg = 실적재 {fmt(liveR.net)}kg</span>
                          <span style={{color:bc}}>{pct.toFixed(1)}%</span>
                        </div>
                      </div>
                    );
                  })()}
                  ...
                  {estQ?(
                    <>
                      <div style={{fontSize:11,color:C.textSub,fontWeight:600,marginBottom:6}}>{exactMatch?"실적값":"추정값"} <span style={S.tag(exactMatch?"amber":"green")}>{exactMatch?`DB ${exactCount}개 평균`:`보정비율 ${corrRatio?.toFixed(2)}x`}</span></div>
                      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:10}}>
                        <div style={S.resultCard(color)}><div style={{fontSize:11,color:C.textSub,marginBottom:4}}>{exactMatch?"실적 적입수량":"추정 적입수량"}</div><div style={{fontSize:22,fontWeight:700,color}}>{fmt(estQ)}</div><div style={{fontSize:11,color:C.textMuted}}>{exactMatch?`DB ${exactCount}개 평균`:`×${corrRatio?.toFixed(2)}`}</div></div>
                        <div style={S.resultCard(color)}><div style={{fontSize:11,color:C.textSub,marginBottom:4}}>{exactMatch?"실적 개당 단가":"추정 개당 단가"}</div><div style={{fontSize:22,fontWeight:700,color}}>₩{fmtP(eUC)}</div><div style={{fontSize:11,color:C.textMuted}}>파렛트 {fmt(ePC)}개</div></div>
                        <div style={S.resultCard(color)}><div style={{fontSize:11,color:C.textSub,marginBottom:4}}>연간 포장비 (추정)</div><div style={{fontSize:20,fontWeight:700,color}}>₩{fmt(p.price*ePC)}</div><div style={{fontSize:11,color:C.textMuted}}>{aq>0?fmt(aq)+"개 기준":""}</div></div>
                      </div>
                      {liveR.mass>0&&(()=>{
                        const estMass=estQ*liveR.mass;
                        const ePct=Math.min(100,estMass/liveR.net*100);
                        const ebc=ePct>=100?C.red:ePct>=80?C.amber:C.green;
                        return(
                          <div style={S.weightBar}>
                            <div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:C.textSub,marginBottom:2}}>
                              <span>{exactMatch?"실적값":"추정값"} 기준 적재 중량</span>
                              <span style={{color:ebc,fontWeight:600}}>{fmtD(estMass)}kg / {fmt(liveR.net)}kg</span>
                            </div>
                            <div style={S.barTrack}><div style={{height:"100%",width:`${ePct}%`,background:ebc,borderRadius:3}}/></div>
                            <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:C.textMuted}}>
                              <span>{fmt(estQ)}개 × {liveR.mass}kg = {fmtD(estMass)}kg</span>
                              <span style={{color:ebc}}>{ePct.toFixed(1)}%</span>
                            </div>
                          </div>
                        );
                      })()}
                    ...
                    <div style={{background:"#fff7ed",border:"1px solid #fed7aa",borderRadius:8,padding:"12px",display:"flex",gap:10}}>
                      <span>⚠️</span><div><div style={{fontSize:13,fontWeight:600,color:"#9a3412",marginBottom:2}}>데이터베이스에 비교군이 없습니다</div><div style={{fontSize:12,color:"#c2410c"}}>대분류 "{item.category}" / 소분류 "{item.tag}"</div></div>
                    </div>
                  ):null}
                </div>
              );
        ...
        <div>
          ...
          <div style={S.section}>
            ...
            <div style={{marginBottom:10}}>
              <label style={S.label}>소분류</label>
              {quoteTags.length===0?(
                <div style={{fontSize:12,color:C.textMuted,marginTop:6}}>대분류를 먼저 선택하세요</div>
              ):(
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(90px,1fr))",gap:8,marginTop:6}}>
                  {quoteTags.map(t=>{
                    const img=tagImages?.[t];
                    const isSel=quote.tag===t;
                    return(
                      <div key={t} onClick={()=>setQ("tag",t)} style={{cursor:"pointer",borderRadius:8,border:`2px solid ${isSel?C.blue:C.border}`,background:isSel?"#eff6ff":"#f8fafc",overflow:"hidden",transition:"all 0.15s"}}>
                        {img?(
                          <img src={img} style={{width:"100%",height:64,objectFit:"cover",display:"block"}}/>
                        ):(
                          <div style={{width:"100%",height:64,background:"#e2e8f0",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22}}>📦</div>
                        )}
                        <div style={{padding:"5px 6px",textAlign:"center",fontSize:11,fontWeight:isSel?700:400,color:isSel?C.blue:C.text,lineHeight:1.3,wordBreak:"break-all"}}>{t}</div>
                      </div>
                    );
                  })}
                </div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              ...
              <button onClick={()=>{
                if(!quote.w||!quote.d||!quote.h){alert("가로/세로/높이를 입력하세요.");return;}
                if(!quote.palletId){alert("파렛트를 선택하세요.");return;}
                if(!quote.annualQty){alert("연간생산수량을 입력하세요.");return;}
              }} style={{...S.btn("primary"),marginLeft:"auto",padding:"9px 24px",fontSize:14}}>
                🔢 계산
              </button>
            </div>

            {/* 신규 제품 인라인 결과 */}
            {qResult&&qResult.qpp>0&&+quote.annualQty>0&&(()=>{
              const{p,best,qpp,corrRatio,estQ,net,mass,aq,exactMatch,exactCount,tagMatchCount,noData}=qResult;
              const cPC=Math.ceil(aq/qpp),cUC=parseFloat((p.price/qpp).toFixed(2));
              const ePC=estQ?Math.ceil(aq/estQ):0,eUC=estQ?parseFloat((p.price/estQ).toFixed(2)):0;
              const color=exactMatch?C.amber:C.green;
              return(
                <div style={{marginTop:14,borderTop:`1px solid ${C.border}`,paddingTop:14}}>
                  {/* 이론값 */}
                  <div style={{fontSize:11,color:C.textSub,fontWeight:600,marginBottom:8}}>
                    이론값 <span style={S.tag("blue")}>{best.l} 방향</span>
                    {best.lim&&<span style={{...S.tag("amber"),marginLeft:6}}>중량 제한</span>}
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:10}}>
                    <div style={S.resultCard(C.blue)}><div style={{fontSize:11,color:C.textSub,marginBottom:4}}>이론 적입수량</div><div style={{fontSize:22,fontWeight:700,color:C.blue}}>{fmt(qpp)}</div><div style={{fontSize:11,color:C.textMuted}}>개/파렛트 · {best.cW}×{best.cD}×{best.cH}</div></div>
                    <div style={S.resultCard(C.blue)}><div style={{fontSize:11,color:C.textSub,marginBottom:4}}>이론 개당 단가</div><div style={{fontSize:22,fontWeight:700,color:C.blue}}>₩{fmtP(cUC)}</div><div style={{fontSize:11,color:C.textMuted}}>{p.name}</div></div>
                    <div style={S.resultCard(C.blue)}><div style={{fontSize:11,color:C.textSub,marginBottom:4}}>이론 연간 포장비</div><div style={{fontSize:20,fontWeight:700,color:C.blue}}>₩{fmt(p.price*cPC)}</div><div style={{fontSize:11,color:C.textMuted}}>파렛트 {fmt(cPC)}개</div></div>
                  </div>
                  {/* 이론값 기준 중량 바 */}
                  {mass>0&&(()=>{
                    const tw=qpp*mass,pct=Math.min(100,tw/net*100);
                    const bc=pct>=100?C.red:pct>=80?C.amber:C.green;
                    return(
                      <div style={{...S.weightBar,marginBottom:10}}>
                        <div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:C.textSub,marginBottom:2}}><span>제품 적재 중량 (이론값 기준)</span><span style={{color:bc,fontWeight:600}}>{fmtD(tw)}kg / {fmt(net)}kg</span></div>
                        <div style={S.barTrack}><div style={{height:"100%",width:`${pct}%`,background:bc,borderRadius:3}}/></div>
                        <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:C.textMuted}}><span>한계 {fmt(p.maxKg)}kg − 자중 {p.tare}kg = 실적재 {fmt(net)}kg</span><span style={{color:bc}}>{pct.toFixed(1)}%</span></div>
                      </div>
                    );
                  })()}
                  {/* 추정값 */}
                  {estQ?(
                    <>
                      <div style={{fontSize:11,color:C.textSub,fontWeight:600,marginBottom:8}}>
                        {exactMatch?"실적값":"추정값"} <span style={S.tag(exactMatch?"amber":"green")}>{exactMatch?`DB ${exactCount}개 평균`:`보정비율 ${corrRatio?.toFixed(2)}x · ${tagMatchCount}개 기반`}</span>
                      </div>
                      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:10}}>
                        <div style={S.resultCard(color)}><div style={{fontSize:11,color:C.textSub,marginBottom:4}}>{exactMatch?"실적 적입수량":"추정 적입수량"}</div><div style={{fontSize:22,fontWeight:700,color}}>{fmt(estQ)}</div><div style={{fontSize:11,color:C.textMuted}}>{exactMatch?`DB ${exactCount}개 평균`:`이론값 × ${corrRatio?.toFixed(2)}`}</div></div>
                        <div style={S.resultCard(color)}><div style={{fontSize:11,color:C.textSub,marginBottom:4}}>{exactMatch?"실적 개당 단가":"추정 개당 단가"}</div><div style={{fontSize:22,fontWeight:700,color}}>₩{fmtP(eUC)}</div><div style={{fontSize:11,color:C.textMuted}}>{p.name}</div></div>
                        <div style={S.resultCard(color)}><div style={{fontSize:11,color:C.textSub,marginBottom:4}}>연간 포장비 (추정)</div><div style={{fontSize:20,fontWeight:700,color}}>₩{fmt(p.price*ePC)}</div><div style={{fontSize:11,color:C.textMuted}}>파렛트 {fmt(ePC)}개</div></div>
                      </div>
                      {mass>0&&(()=>{
                        const estMass=estQ*mass,ePct=Math.min(100,estMass/net*100);
                        const ebc=ePct>=100?C.red:ePct>=80?C.amber:C.green;
                        return(
                          <div style={S.weightBar}>
                            <div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:C.textSub,marginBottom:2}}><span>{exactMatch?"실적값":"추정값"} 기준 적재 중량</span><span style={{color:ebc,fontWeight:600}}>{fmtD(estMass)}kg / {fmt(net)}kg</span></div>
                            <div style={S.barTrack}><div style={{height:"100%",width:`${ePct}%`,background:ebc,borderRadius:3}}/></div>
