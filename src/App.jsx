...
function calcOneProduct(prod, db){
  ...
  // 근사 제품 매칭 (85~95%) → 보정비율 적용
  // 각 유사 제품의 (실적 적입수 ÷ 해당 제품의 이론 계산값) = 보정비율
  // 신규 제품 추정 = 신규 이론값 × 평균 보정비율
  const fillSource=filterForFillRate(prod,db);
  if(fillSource.length===0){
    return{prod,p,best,qpp:best.qpp,estQ:null,corrRatio:null,exactMatch:false,tagMatchCount:0,noData:true,pVol,pv,net,mass,aq,scored};
  }

  // 각 유사 제품의 보정비율 계산
  const ratios=fillSource.map(m=>{
    const mOrients=getOrients(p,[+m.w,+m.d,+m.h],+m.mass||0);
    const mBest=getBest(mOrients);
    if(mBest.qpp<=0) return null;
    return m.actual/mBest.qpp; // 실적 ÷ 이론값
  }).filter(r=>r!==null&&r>0);

  if(ratios.length===0){
    return{prod,p,best,qpp:best.qpp,estQ:null,corrRatio:null,exactMatch:false,tagMatchCount:fillSource.length,noData:true,pVol,pv,net,mass,aq,scored};
  }
...
export default function App(){
  ...
  const[palletPrices,setPalletPrices]=useState(()=>{
    const init={};
    PALLETS.forEach(p=>{init[p.id]=p.price;});
    return init;
  });
  ...
  function startEditPrice(e,pId,currentPrice){
    e.stopPropagation();
    setEditingPriceId(pId);
    setEditingPriceVal(String(currentPrice));
  }
  function commitPrice(pId){
    const v=parseInt(editingPriceVal.replace(/,/g,""))||0;
    if(v>0) setPalletPrices(prev=>({...prev,[pId]:v}));
  ...
  function downloadSample(){
    ...
    // 태그 목록 시트 추가
    const tagRows=allTags.map(t=>({제품구분:t}));
    const wsTag=XLSX.utils.json_to_sheet(tagRows.length>0?tagRows:[{제품구분:"(DB에 등록된 구분 없음)"}]);
    wsTag["!cols"]=[{wch:20}];

    const wb=XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb,ws,"입력양식");
    XLSX.utils.book_append_sheet(wb,wsTag,"제품구분목록");
  ...
  useEffect(()=>{
    ...
    // 2. 태그+형상+부피 3요소 필터로 보정비율 계산
    const fillSource=filterForFillRate({tag:quote.tag,w:pw,d:pd,h:ph},matched);
    if(fillSource.length===0){
      setQResult({p,orients,best,qpp:best.qpp,corrRatio:null,estQ:null,pVol,pv,net,mass,aq,exactMatch:false,tagMatchCount:0,noData:true});
      return;
    }
    // 각 유사 제품의 보정비율 = 실적 ÷ 해당 제품의 이론 계산값
    const ratios=fillSource.map(m=>{
      const mOrients=getOrients(p,[+m.w,+m.d,+m.h],+m.mass||0);
      const mBest=getBest(mOrients);
      if(mBest.qpp<=0) return null;
      return m.actual/mBest.qpp;
    }).filter(r=>r!==null&&r>0);
    if(ratios.length===0){
      setQResult({p,orients,best,qpp:best.qpp,corrRatio:null,estQ:null,pVol,pv,net,mass,aq,exactMatch:false,tagMatchCount:fillSource.length,noData:true});
      return;
    }
    const corrRatio=ratios.reduce((a,b)=>a+b,0)/ratios.length;
    let estQ=Math.round(best.qpp*corrRatio);
  ...
  // KPI 계산
  const selectedPalletRaw=PALLETS.find(x=>x.id===quote.palletId);
  const selectedPallet=selectedPalletRaw?getPalletWithPrice(selectedPalletRaw):null;
  const kpiQpp=qResult?.qpp||0;
  const kpiEstQ=qResult?.estQ||null;
  const kpiUnitCost=kpiQpp>0&&selectedPallet?Math.round(selectedPallet.price/kpiQpp):0;
  const kpiEstUnitCost=kpiEstQ&&selectedPallet?Math.round(selectedPallet.price/kpiEstQ):0;
  const kpiAq=+quote.annualQty||0;
  // 연간 총 포장비 = 추정 단가 기준 (추정값 없으면 계산값 기준)
  const kpiUseQ=kpiEstQ||kpiQpp;
  const kpiTotalCost=kpiUseQ>0&&selectedPallet&&kpiAq?selectedPallet.price*Math.ceil(kpiAq/kpiUseQ):0;
  const kpiCorrRatio=qResult?.corrRatio||null;
        ...
        {tab==="quote"?(
          <>
            ...
            <div style={{...S.kpiGrid,gridTemplateColumns:"repeat(5,1fr)"}}>
              ...
              <KpiCard label="이론 개당 단가" value={kpiUnitCost>0?`₩${fmt(kpiUnitCost)}`:"—"} unit={selectedPallet?.name||""} color={C.blue}/>
              <KpiCard label="추정 적입수량" value={kpiEstQ?fmt(kpiEstQ):"—"} unit={kpiEstQ?"개/파렛트":""} color={C.green} sub={qResult?.exactMatch?"DB 실적값 직접 적용":kpiCorrRatio?`보정비율 ${kpiCorrRatio.toFixed(2)}x`:undefined}/>
              <KpiCard label="추정 개당 단가" value={kpiEstUnitCost>0?`₩${fmt(kpiEstUnitCost)}`:"—"} unit={selectedPallet?.name||""} color={C.green}/>
              <KpiCard label="연간 총 포장비" value={kpiTotalCost>0?`₩${fmt(Math.round(kpiTotalCost/10000))}만`:"—"} unit={kpiTotalCost>0?`파렛트 ${fmt(Math.ceil(kpiAq/(kpiUseQ||1)))}개`:""} color={C.amber} sub="추정 단가 기준"/>
            </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 340px",gap:20}}>
              <div>
                ...
                <div style={S.section}>
                  ...
                  <div style={{...S.grid6,marginBottom:10}}>
                  ...
                  <div style={{marginBottom:12}}>
                    <div style={{fontSize:11,color:C.textSub,fontWeight:600,marginBottom:6}}>구분 선택 (클릭)</div>
                    <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                      {allTags.map(t=>(
                        <button key={t} onClick={()=>setQ("tag",t)}
                          style={{fontSize:11,padding:"3px 10px",borderRadius:20,border:`1px solid ${quote.tag===t?C.blue:C.border}`,background:quote.tag===t?"#dbeafe":"#f8fafc",color:quote.tag===t?C.blue:C.textSub,cursor:"pointer",fontWeight:quote.tag===t?600:400,transition:"all 0.1s"}}>
                          {t}
                        </button>
                      ))}
                    </div>
                          ...
                          {isEditing?(
                            <div style={{display:"flex",alignItems:"center",gap:4}} onClick={e=>e.stopPropagation()}>
                              ...
                              <input
                                ...
                                onKeyDown={e=>{if(e.key==="Enter")commitPrice(p.id);if(e.key==="Escape")setEditingPriceId(null);}}
                                onBlur={()=>commitPrice(p.id)}
                                style={{...S.input,width:"100%",fontSize:13,fontWeight:700,padding:"3px 6px",color:C.blue}}
                              />
                            </div>
                          ):(
                            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                              <div style={{fontSize:14,fontWeight:700,color:sel?C.blue:C.textSub}}>
                                ₩{fmt(pw.price)}
                                {pw.price!==p.price&&<span style={{fontSize:10,color:C.amber,marginLeft:4}}>수정됨</span>}
                              </div>
                              <button
                                onClick={e=>startEditPrice(e,p.id,pw.price)}
                                style={{background:"none",border:`1px solid ${C.border}`,borderRadius:4,cursor:"pointer",fontSize:11,color:C.textMuted,padding:"2px 6px"}}
                              >수정</button>
                            </div>
                          )}
                        </div>
                        ...
                        return(
                          <>
                            <div style={{marginBottom:8,fontSize:12,color:C.textSub}}>
                              이론값 <span style={S.tag("blue")}>최적 방향 이론치</span>{best.lim&&<span style={{...S.tag("amber"),marginLeft:6}}>중량 제한</span>}
                            </div>
                            <div style={S.resultGrid}>
                              <div style={S.resultCard(C.blue)}><div style={{fontSize:11,color:C.textSub,marginBottom:6}}>이론 적입수량</div><div style={{fontSize:22,fontWeight:700,color:C.blue}}>{fmt(qpp)}</div><div style={{fontSize:11,color:C.textMuted,marginTop:2}}>개/파렛트 · {best.cW}×{best.cD}×{best.cH}</div></div>
                              <div style={S.resultCard(C.blue)}><div style={{fontSize:11,color:C.textSub,marginBottom:6}}>이론 개당 단가</div><div style={{fontSize:22,fontWeight:700,color:C.blue}}>₩{fmt(cUC)}</div><div style={{fontSize:11,color:C.textMuted,marginTop:2}}>{p.name}</div></div>
                              <div style={S.resultCard(C.blue)}><div style={{fontSize:11,color:C.textSub,marginBottom:6}}>이론 연간 포장비</div><div style={{fontSize:22,fontWeight:700,color:C.blue}}>₩{fmt(cTC)}</div><div style={{fontSize:11,color:C.textMuted,marginTop:2}}>파렛트 {fmt(cPC)}개</div></div>
                            </div>
                            {estQ!==null?(()=>{
                              const ePC=Math.ceil(aq/estQ),eUC=p.price/estQ,eTC=p.price*ePC;
                              const isExact=exactMatch;
                              const color=isExact?C.amber:C.green;
                              const corrLabel=isExact?`DB ${exactCount}개 평균`:(corrRatio?`이론값 × ${corrRatio.toFixed(2)}`:"-");
                              return(
                                <>
                                  <hr style={S.divider}/>
                                  <div style={{marginBottom:8,fontSize:12,color:C.textSub}}>
                                    {isExact
                                      ...
                                      :<>추정값 <span style={S.tag("green")}>보정비율 {corrRatio?.toFixed(2)}x · {tagMatchCount}개 유사 제품 기반</span></>
                                    }
                                  </div>
                                  <div style={S.resultGrid}>
                                    <div style={S.resultCard(color)}><div style={{fontSize:11,color:C.textSub,marginBottom:6}}>{isExact?"실적 적입수량":"추정 적입수량"}</div><div style={{fontSize:22,fontWeight:700,color}}>{fmt(estQ)}</div><div style={{fontSize:11,color:C.textMuted,marginTop:2}}>{corrLabel}</div></div>
                                    ...
                                    <div style={S.resultCard(color)}><div style={{fontSize:11,color:C.textSub,marginBottom:6}}>연간 포장비 (추정 기준)</div><div style={{fontSize:22,fontWeight:700,color}}>₩{fmt(eTC)}</div><div style={{fontSize:11,color:C.textMuted,marginTop:2}}>파렛트 {fmt(ePC)}개</div></div>
                                  </div>
                                </>
                              );
                            })():(
                            ...
                            {estQ!==null&&mass>0&&(()=>{
                              ...
                              const pct=Math.min(100,estMass/net*100);
                              const bc=pct>=100?C.red:pct>=80?C.amber:C.green;
                              const isExact=exactMatch;
                              return(
                                <div style={{...S.weightBar,marginBottom:12}}>
                                  <div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:C.textSub,marginBottom:2}}>
                                    <span>제품 적재 중량 ({isExact?"실적값":"추정값"} 기준)</span>
                                    <span style={{color:bc,fontWeight:600}}>{parseFloat(estMass.toFixed(1)).toLocaleString()}kg / {fmt(net)}kg</span>
                                  </div>
                                  <div style={S.barTrack}><div style={{height:"100%",width:`${pct}%`,background:bc,borderRadius:3,transition:"width 0.4s"}}/></div>
                                  <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:C.textMuted}}>
                                    <span>{fmt(estQ)}개 × {mass}kg = {parseFloat(estMass.toFixed(1)).toLocaleString()}kg</span>
                                    <span style={{color:bc}}>{pct.toFixed(1)}% 사용</span>
                                  </div>
                                </div>
                              );
                            ...
                            <div style={{fontSize:11,color:C.textSub,lineHeight:2}}>
                              ...
                              최적 ({best.l}): {best.cW}×{best.cD}×{best.cH} = {fmt(best.qs)}개 = <b style={{color:C.text}}>{fmt(qpp)}개/파렛트 (이론)</b>
                              {estQ!==null&&(()=>{
                                const rawEst=corrRatio?Math.round(qpp*corrRatio):estQ;
                                const limited=mass>0&&net>0&&estQ<rawEst;
                                return<><br/>{exactMatch
                                  ?`DB 동일 제품 ${exactCount}개 평균 → 실적값 적용 `
                                  :`이론값 ${fmt(qpp)} × ${corrRatio?.toFixed(2)} = ${fmt(rawEst)}개`
                                }{limited?<> → <span style={{color:C.amber}}>중량 제한({fmt(net)}kg ÷ {mass}kg)</span> = </> : " → "}<b style={{color:C.text}}>{fmt(estQ)}개/파렛트 (추정)</b></>;
                              })()}
                            </div>
                          </>
                        );
              ...
              <div>
                <div style={S.section}>
                  <div style={S.sectionTitle}>
                    <span style={{width:3,height:14,background:C.green,borderRadius:2,display:"inline-block"}}/>
                    유사 제품 매칭
                    {qResult?.tagMatchCount>0&&<span style={{marginLeft:"auto",fontSize:11,color:C.textMuted,fontWeight:400}}>{qResult.tagMatchCount}개 반영</span>}
                  </div>
                  {matched.length===0?(
                    ...
                    // 실제 보정비율 계산에 사용된 제품만 표시
                    const displayList=qResult?.exactMatch
                      ? matched.filter(m=>m.exactMatch)  // 동일 제품
                      : filterForFillRate({tag:quote.tag,w:+quote.w,d:+quote.d,h:+quote.h},matched); // 3요소 필터 적용 제품
                    return(
                    <>
                      {nf&&(
                      ...
                      {displayList.length===0?(
                        <div style={{textAlign:"center",padding:"1.5rem 1rem",color:C.textMuted,fontSize:12}}>
                          {quote.tag?"태그 일치 제품 없음":"태그를 입력하세요"}
                        </div>
                      ):(
                        <div style={{marginBottom:10}}>
                          {displayList.map((m,i)=>{
                            ...
                            // 해당 제품의 이론값과 보정비율 계산
                            const mOrients=qResult?.p?getOrients(qResult.p,[+m.w,+m.d,+m.h],+m.mass||0):null;
                            const mBest=mOrients?getBest(mOrients):null;
                            const mRatio=mBest&&mBest.qpp>0?(m.actual/mBest.qpp):null;
                            return(
                              <div key={m.id||i} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 12px",borderRadius:8,background:isExact?"#fffbeb":"#f8fafc",border:`1px solid ${isExact?"#fcd34d":C.border}`,marginBottom:6}}>
                                <span style={{fontSize:11,fontWeight:700,color:isExact?C.amber:C.textMuted,minWidth:20}}>#{i+1}</span>
                                <div style={{flex:1,minWidth:0}}>
                                  <div style={{fontSize:12,fontWeight:600,color:isExact?"#92400e":C.text,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>
                                    {m.tag||"—"}{isExact&&<span style={{fontSize:10,background:"#fef3c7",color:"#92400e",borderRadius:3,padding:"1px 5px",marginLeft:5,fontWeight:700}}>동일</span>}
                                    {mf&&<span style={{fontSize:10,color:C.textMuted,fontWeight:400,marginLeft:4}}>· {shapeLabel(mf)}</span>}
                                  </div>
                                  <div style={{fontSize:11,color:C.textSub}}>
                                    {m.w}×{m.d}×{m.h} · <span style={{color:C.green,fontWeight:600}}>{fmt(m.actual)}개</span>
                                    {mRatio&&<span style={{color:C.textMuted}}> · 보정 {mRatio.toFixed(2)}x</span>}
                                  </div>
                                </div>
                                {mp&&<span style={{fontSize:10,color:C.textMuted,whiteSpace:"nowrap"}}>{mp.name}</span>}
                              </div>
                            );
                          })}
                        </div>
                      ...
                      {qResult?.exactMatch?(
                        ...
                        <div style={{background:"#f0fdf4",border:"1px solid #86efac",borderRadius:8,padding:"12px",textAlign:"center"}}>
                          <div style={{fontSize:11,color:"#15803d",marginBottom:4}}>평균 보정비율</div>
                          <div style={{fontSize:24,fontWeight:700,color:C.green}}>{qResult.corrRatio.toFixed(2)}x</div>
                          <div style={{fontSize:11,color:"#15803d",marginTop:2}}>{qResult.tagMatchCount}개 유사 제품 기반</div>
                          <div style={{fontSize:10,color:C.textMuted,marginTop:3}}>태그 + 형상비율 + 부피 ±50%</div>
                        </div>
                      ):quote.tag?(
