        ...
        <div>
          ...
          {qResult&&(
            <div style={S.section}>
              ...
              {/* 견적 결과 카드 - 방향별 분석 하단 */}
              {qResult.qpp>0&&qResult.aq>0&&(()=>{
                const{p,best,qpp,corrRatio,estQ,net,mass,aq,exactMatch,exactCount,tagMatchCount,noData}=qResult;
                const cPC=Math.ceil(aq/qpp),cUC=parseFloat((p.price/qpp).toFixed(2)),cTC=p.price*cPC;
                return(
                  <>
                    <hr style={S.divider}/>
                    <div style={{fontSize:11,color:C.textSub,fontWeight:600,marginBottom:8}}>이론값 <span style={S.tag("blue")}>최적 방향</span>{best.lim&&<span style={{...S.tag("amber"),marginLeft:6}}>중량 제한</span>}</div>
                    <div style={S.resultGrid}>
                      <div style={S.resultCard(C.blue)}><div style={{fontSize:11,color:C.textSub,marginBottom:4}}>이론 적입수량</div><div style={{fontSize:22,fontWeight:700,color:C.blue}}>{fmt(qpp)}</div><div style={{fontSize:11,color:C.textMuted}}>개/파렛트</div></div>
                      ...
                      <div style={S.resultCard(C.blue)}><div style={{fontSize:11,color:C.textSub,marginBottom:4}}>이론 연간 포장비</div><div style={{fontSize:20,fontWeight:700,color:C.blue}}>₩{fmt(cTC)}</div><div style={{fontSize:11,color:C.textMuted}}>파렛트 {fmt(cPC)}개</div></div>
                    </div>
                    {estQ!==null?(()=>{
                      const ePC=Math.ceil(aq/estQ),eUC=parseFloat((p.price/estQ).toFixed(2)),eTC=p.price*ePC;
                      ...
                      return(
                        <>
                          <hr style={S.divider}/>
                          <div style={{fontSize:11,color:C.textSub,fontWeight:600,marginBottom:8}}>{isExact?"실적값":"추정값"} <span style={S.tag(isExact?"amber":"green")}>{isExact?`DB ${exactCount}개 평균`:`보정비율 ${corrRatio?.toFixed(2)}x · ${tagMatchCount}개 기반`}</span></div>
                          <div style={S.resultGrid}>
                            <div style={S.resultCard(color)}><div style={{fontSize:11,color:C.textSub,marginBottom:4}}>{isExact?"실적 적입수량":"추정 적입수량"}</div><div style={{fontSize:22,fontWeight:700,color}}>{fmt(estQ)}</div><div style={{fontSize:11,color:C.textMuted}}>{isExact?`DB ${exactCount}개 평균`:`×${corrRatio?.toFixed(2)}`}</div></div>
                            <div style={S.resultCard(color)}><div style={{fontSize:11,color:C.textSub,marginBottom:4}}>{isExact?"실적 개당 단가":"추정 개당 단가"}</div><div style={{fontSize:22,fontWeight:700,color}}>₩{fmtP(eUC)}</div><div style={{fontSize:11,color:C.textMuted}}>{p.name}</div></div>
                            <div style={S.resultCard(color)}><div style={{fontSize:11,color:C.textSub,marginBottom:4}}>연간 포장비 (추정)</div><div style={{fontSize:20,fontWeight:700,color}}>₩{fmt(eTC)}</div><div style={{fontSize:11,color:C.textMuted}}>파렛트 {fmt(ePC)}개</div></div>
                          </div>
                          {mass>0&&(
                            <div style={{...S.weightBar,marginTop:10}}>
                              <div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:C.textSub,marginBottom:2}}><span>추정값 기준 적재 중량</span><span style={{color:ebc,fontWeight:600}}>{fmtD(estMass)}kg / {fmt(net)}kg</span></div>
