...
export default function App(){
  ...
  function downloadSample(){
    const XLSX=window.XLSX;if(!XLSX){alert("엑셀 라이브러리 로딩 중입니다.");return;}
    const sample=[
      {품번:"54530-S1000",품명:"B/JOINT-ASS'Y",대분류:"ARM",소분류:"B/JOINT",가로:145,세로:72,높이:82,중량:0.769,연간수량:10000},
      {품번:"55280-S1000W",품명:"RR T/ARM ASSY,LH",대분류:"ARM",소분류:"T/ARM RR",가로:497,세로:181,높이:61,중량:1.478,연간수량:8000},
    ];
    const ws=XLSX.utils.json_to_sheet(sample);
    ws["!cols"]=[{wch:16},{wch:28},{wch:12},{wch:14},{wch:8},{wch:8},{wch:8},{wch:8},{wch:10}];
    // 대분류 드롭다운 (D열→C열 = 대분류)
    const catList=allCategories.join(",");
    // 소분류는 제품구분목록 시트 참조
    const tagRows=db.map(x=>({대분류:x.category||"",소분류:x.tag||""})).filter((v,i,a)=>a.findIndex(t=>t.대분류===v.대분류&&t.소분류===v.소분류)===i).sort((a,b)=>a.대분류.localeCompare(b.대분류)||a.소분류.localeCompare(b.소분류));
    const wsTag=XLSX.utils.json_to_sheet(tagRows.length>0?tagRows:[{대분류:"(없음)",소분류:"(없음)"}]);
    wsTag["!cols"]=[{wch:14},{wch:18}];
    if(allCategories.length>0){
      ws["!dataValidations"]=[
        {sqref:"C2:C1000",type:"list",formula1:`"${catList}"`,showDropDown:false,showErrorMessage:true,errorTitle:"입력 오류",error:"대분류 목록에서 선택하세요."},
        {sqref:"D2:D1000",type:"list",formula1:`제품구분목록!$B$2:$B$${tagRows.length+1}`,showDropDown:false,showErrorMessage:true,errorTitle:"입력 오류",error:"소분류 목록에서 선택하세요."},
      ];
    }
    const wb=XLSX.utils.book_new();
