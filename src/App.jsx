...
export default function App(){
  ...
  function downloadSample(){
    // SheetJS 무료버전은 dataValidation 미지원 → 직접 xlsx XML 생성
    const XLSX=window.XLSX;if(!XLSX){alert("엑셀 라이브러리 로딩 중입니다.");return;}

    const catList=allCategories;
    const tagRows=db.map(x=>({cat:x.category||"",tag:x.tag||""}))
      .filter((v,i,a)=>a.findIndex(t=>t.cat===v.cat&&t.tag===v.tag)===i)
      .sort((a,b)=>a.cat.localeCompare(b.cat)||a.tag.localeCompare(b.tag));
    ...
    // 입력양식 셀 생성
    let sheetXml='<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheetData>';
    sampleData.forEach((row,r)=>{
      sheetXml+=`<row r="${r+1}">`;
      row.forEach((val,c)=>{
        const ref=cellRef(r,c);
        if(typeof val==="number"){sheetXml+=`<c r="${ref}" t="n"><v>${val}</v></c>`;}
        else{const si=addStr(val);sheetXml+=`<c r="${ref}" t="s"><v>${si}</v></c>`;}
      });
      sheetXml+="</row>";
    });
    sheetXml+="</sheetData>";
    ...
    if(catCount>0||tagCount>0){
      ...
      // 대분류 (C열): 직접 목록 (255자 이하면 inline)
      const catInline='"'+catList.map(escXml).join(",")+'"';
      const catFormula=catInline.length<=255
        ?catInline
        :`제품구분목록!$A$2:$A$${catCount+1}`;
      sheetXml+=`<dataValidation type="list" allowBlank="1" showDropDown="0" showErrorMessage="1" errorTitle="입력오류" error="목록에서 선택하세요" sqref="C2:C1000"><formula1>${escXml(catFormula)}</formula1></dataValidation>`;
      // 소분류 (D열): 제품구분목록 시트 B열 참조
      if(tagCount>0){
        sheetXml+=`<dataValidation type="list" allowBlank="1" showDropDown="0" showErrorMessage="1" errorTitle="입력오류" error="목록에서 선택하세요" sqref="D2:D1000"><formula1>제품구분목록!$B$2:$B$${tagCount+1}</formula1></dataValidation>`;
      }
    ...
    // ── 제품구분목록 시트 XML ──────────────────────────────
    let tagSheetXml='<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetData>';
    // 헤더
    const hCat=addStr("대분류"),hTag=addStr("소분류");
    tagSheetXml+=`<row r="1"><c r="A1" t="s"><v>${hCat}</v></c><c r="B1" t="s"><v>${hTag}</v></c></row>`;
    tagRows.forEach(({cat,tag},i)=>{
      const siCat=addStr(cat),siTag=addStr(tag);
      tagSheetXml+=`<row r="${i+2}"><c r="A${i+2}" t="s"><v>${siCat}</v></c><c r="B${i+2}" t="s"><v>${siTag}</v></c></row>`;
    });
...
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
  <sheets>
    ...
    // ── JSZip으로 조립 ─────────────────────────────────────
    // JSZip이 없으면 SheetJS 방식으로 fallback
    if(typeof JSZip==="undefined"){
      // fallback: SheetJS (드롭다운 없이)
      const wb2=XLSX.utils.book_new();
      const ws2=XLSX.utils.aoa_to_sheet(sampleData);
      ws2["!cols"]=[{wch:16},{wch:28},{wch:12},{wch:14},{wch:8},{wch:8},{wch:8},{wch:8},{wch:10}];
      const wsTag2=XLSX.utils.json_to_sheet(tagRows.map(x=>({대분류:x.cat,소분류:x.tag})));
      XLSX.utils.book_append_sheet(wb2,ws2,"입력양식");
      XLSX.utils.book_append_sheet(wb2,wsTag2,"제품구분목록");
      XLSX.writeFile(wb2,"견적입력_양식.xlsx");
      alert("드롭다운 기능을 위해 JSZip 라이브러리가 필요합니다. 페이지를 새로고침 후 다시 시도해주세요.");
      return;
    }
    const zip=new JSZip();
...
// ── DB 탭 ─────────────────────────────────────────────────
function DBTab({db,pageDb,filteredDb,dbPage,totalPages,setDbPage,dbSearch,setDbSearch,dbCatFilter,setDbCatFilter,allCategories,dbForm,setDbForm,allTags,saveDb,deleteDb,editingDbItem,startEditDb,saveEditDb,setEditingDbItem,handleDbImg,PALLETS}){
