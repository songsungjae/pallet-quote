        ...
        <div>
          ...
          {qResult&&(
            <div style={S.section}>
              ...
              {/* 추정값 기준 적재 중량 바 */}
              {qResult.qpp>0&&qResult.aq>0&&qResult.estQ&&qResult.mass>0&&(()=>{
                const{net,mass,estQ,exactMatch}=qResult;
                const estMass=estQ*mass;
                const ePct=Math.min(100,estMass/net*100);
                const ebc=ePct>=100?C.red:ePct>=80?C.amber:C.green;
                return(
                  <div style={{...S.weightBar,marginTop:8}}>
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:C.textSub,marginBottom:2}}>
                      <span>{exactMatch?"실적값":"추정값"} 기준 적재 중량</span>
                      <span style={{color:ebc,fontWeight:600}}>{fmtD(estMass)}kg / {fmt(net)}kg</span>
                    </div>
                    <div style={S.barTrack}><div style={{height:"100%",width:`${ePct}%`,background:ebc,borderRadius:3}}/></div>
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:C.textMuted}}>
                      <span>{fmt(estQ)}개 × {mass}kg = {fmtD(estMass)}kg</span>
                      <span style={{color:ebc}}>{ePct.toFixed(1)}%</span>
                    </div>
                  </div>
                );
              })()}
              {qResult.noData&&qResult.qpp>0&&(
                <div style={{background:"#fff7ed",border:"1px solid #fed7aa",borderRadius:8,padding:"12px",display:"flex",gap:10,marginTop:8}}>
